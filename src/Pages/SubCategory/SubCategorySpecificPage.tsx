...
import { useGetProductsySubCategory } from "../../ApiCalls/getProductsBySubCategory";
...
const SubCategorySpecificPage = () => {
  ...
  const { specificSubCategory } = location.state || {specificSubCategory: []}
  // console.log("sub specific", specificSubCategory?._id);
  const [selectedPriceRanges, setSelectedPriceRanges] = useState<string[]>([]);
  const [searchTerm, setSearchTerm] = useState<string>("");
  const { isLoading: isLoadingMale, data: productsDataMale } =
    useGetProductsySubCategory(specificSubCategory?._id, "male");
  const { isLoading: isLoadingFemale, data: productsDataFemale } =
  useGetProductsySubCategory(specificSubCategory?._id, "female");
    ...
    >
      <div className="women-placeholder text-center py-3  lg:py-5 text-white">
        <h1 className="text-[1.3rem] ssm:text-[1.5rem] sm:text-[1.8rem] md:text-[2.2rem] lg:text-[2.5rem]">
          {gender === "male" ? specificSubCategory?.name : specificSubCategory?.name}
        </h1>
      </div>
...
export default SubCategorySpecificPage;
